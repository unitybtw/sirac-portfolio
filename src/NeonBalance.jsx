...
const playSound = (type) => {
  ...
  useEffect(() => {
    ...
    let cart = {x: 300, y: 350, w: 60, h: 20, dx: 0};
    let pole = {angle: 0.1, aVel: 0, aAcc: 0, len: 100};
    let left=false, right=false; let scoreVal=0;
    ...
    const draw = () => {
        ...
        if(left) cart.dx -= 0.5; if(right) cart.dx += 0.5;
        cart.dx *= 0.9; cart.x += cart.dx;
        
        // Physics for inverted pendulum roughly
        pole.aAcc = 0.005 * Math.sin(pole.angle) - cart.dx * 0.002 * Math.cos(pole.angle);
        pole.aVel += pole.aAcc; pole.angle += pole.aVel;
        
        if(cart.x < 0) { cart.x=0; cart.dx=0; } if(cart.x > canvas.width-cart.w) { cart.x=canvas.width-cart.w; cart.dx=0; }
        
        if(Math.abs(pole.angle) > Math.PI/2) { { playSound('boom'); if (onGameOver) onGameOver(scoreVal); setIsPlaying(false); return; } } // Fell over
        
        scoreVal++; setScore(Math.floor(scoreVal/10));
        
        ctx.fillStyle='#ff00ff'; ctx.fillRect(cart.x, cart.y, cart.w, cart.h);
        
        let px = cart.x+cart.w/2 + Math.sin(pole.angle)*pole.len;
        let py = cart.y - Math.cos(pole.angle)*pole.len;
        ctx.strokeStyle='#fff'; ctx.lineWidth=6; ctx.beginPath(); ctx.moveTo(cart.x+cart.w/2, cart.y); ctx.lineTo(px, py); ctx.stroke();
        ctx.fillStyle='#00f0ff'; ctx.beginPath(); ctx.arc(px, py, 10, 0, Math.PI*2); ctx.fill();
