...
const NeonHelicopter = ({ onGameOver }) => {
...
const playSound = (type) => {
  ...
  useEffect(() => {
        ...
        let p = { y: 200, dy: 0 }, isUp = false, walls = [], frame = 0, curr = 0;

        const dn = () => isUp = true; const up = () => isUp = false;
    ...
    const draw = () => {
      ...
      _lastFrameTime = _now;
            ...
            p.dy += isUp ? -0.4 : 0.4; p.y += p.dy;
            ctx.fillStyle = '#ff00ff'; ctx.fillRect(100, p.y, 30, 20);

            if (p.y < 0 || p.y > canvas.height) { { playSound('boom'); if (onGameOver) onGameOver(score); setIsPlaying(false); return; } }
            if (frame++ % 50 === 0) walls.push({ x: canvas.width, y: Math.random() * (canvas.height - 100), w: 40, h: 100 });

            for (let w of walls) {
                w.x -= 5; ctx.fillStyle = '#00f0ff'; ctx.fillRect(w.x, w.y, w.w, w.h);
                if (100 < w.x + w.w && 130 > w.x && p.y < w.y + w.h && p.y + 20 > w.y) { { playSound('boom'); if (onGameOver) onGameOver(score); setIsPlaying(false); return; } }
            }
    ...
    return <div style={{ position: 'relative', width: '100%', height: '100%', background: '#050508' }}>
        ...
        {isPlaying ? <div style={{ position: 'absolute', top: 10, left: 10, color: '#fff', fontFamily: 'monospace' }}>Score: {score}</div> :
            <div style={{ position: 'absolute', inset: 0, display: 'flex', justifyContent: 'center', alignItems: 'center', background: 'rgba(0,0,0,0.8)', flexDirection: 'column' }}>
                <h2 className="text-gradient" style={{ marginBottom: '1rem' }}>Neon Copter</h2>
                {score > 0 && <p style={{ color: '#ff00ff', marginBottom: '1rem' }}>Score: {score}</p>}
                <button className="btn btn-primary" onClick={() => { setScore(0); setIsPlaying(true); }}><Play size={18} /> PLAY</button>
            </div>}
    </div>;
};
export default NeonHelicopter;
