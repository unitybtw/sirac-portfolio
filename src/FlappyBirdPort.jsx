...
export default function FlappyBirdPort() {
    const [started, setStarted] = useState(true);
    const iframeRef = useRef(null);

    const toggleFullScreen = () => {
        const elem = document.getElementById("flappy-game-wrapper");
        if (!elem) return;
    ...
    return (
        <div id="flappy-game-wrapper" style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', background: '#000', position: 'relative' }}>
            {!started ? (
                <div style={{ flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '20px', background: '#050508' }}>
                    ...
                    >
                        <h2 className="text-gradient" style={{ fontSize: '2.5rem', marginBottom: '1rem', color: '#55aa55', fontFamily: 'monospace' }}>FLAPPY BIRD</h2>
                        <div style={{ background: 'rgba(85, 170, 85, 0.05)', padding: '1rem', borderRadius: '8px', border: '1px solid rgba(85, 170, 85, 0.2)', marginBottom: '1.5rem', textAlign: 'left', fontSize: '0.9rem' }}>
                            <p style={{ color: '#55aa55', fontWeight: 'bold', marginBottom: '0.8rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                <MousePointer2 size={16} /> CONTROLS / KONTROLLER
                            </p>
                            <ul style={{ color: '#bbb', margin: 0, paddingLeft: '1.2rem', fontSize: '0.8rem', lineHeight: '1.6' }}>
                                <li><strong>SPACE / CLICK:</strong> Fly Up / Zıpla</li>
                            </ul>
                        </div>
                        <button
                ...
                <div style={{ flex: 1, position: 'relative', overflow: 'hidden' }}>
                    <iframe
                        ref={iframeRef}
                        src="https://wayou.github.io/FlappyBird/"
